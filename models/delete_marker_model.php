...
$sql_delete_marker_from_community = "DELETE FROM markers_to_communities WHERE marker_id = '$marker_id'";

$sql_delete_profile_to_marker = "DELETE FROM profiles_to_markers WHERE marker_id = '$marker_id'";

if(mysqli_query($conn, $sql_delete_marker) && mysqli_query($conn, $sql_delete_marker_from_community) && mysqli_query($conn, $sql_delete_profile_to_marker)) {
    echo "success"; exit();
} else {
    echo "failed"; exit();
}
